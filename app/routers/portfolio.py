...
@router.get(
    ...
    summary="포트폴리오 조회",
    responses={200: {"description": "포트폴리오 목록 반환"}},
)
...
def create_portfolio(
    user_id: int, portfolio_data: PortfolioCreate, db: Session = Depends(get_db)
):
    """
    새로운 포트폴리오를 생성.
    ...
    new_portfolio = Portfolio(
        user_id=user_id, portfolio_name=portfolio_data.portfolio_name
    )
...
def update_portfolio(
    ...
    portfolio = (
        db.query(Portfolio).filter(Portfolio.portfolio_id == portfolio_id).first()
    )
    if not portfolio:
    ...
    # 포트폴리오 수정 시 중복 체크
    if (
        portfolio_data.portfolio_name is not None
        and portfolio_data.portfolio_name != portfolio.portfolio_name
    ):
        duplicate_portfolio = (
            db.query(Portfolio)
            .filter(
...
def delete_portfolio(portfolio_id: int, db: Session = Depends(get_db)):
    ...
    if not portfolio:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="포트폴리오 찾을 수 없음"
        )
    db.delete(portfolio)
    db.commit()
